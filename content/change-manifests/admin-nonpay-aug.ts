...
export const ADMIN_NONPAY_AUG_CHANGES: PolicyChange[] = [
  ...
  },
  {
    ...
    after: '연동 병원이면서 자동 확정이 OFF일 때, `예약 자동 확정` 설정 박스 안에 안내 블록을 표시합니다. 제목·설명·토글 한 줄 아래에 전체 폭으로 붙습니다. 제목과 세 개의 안내(굿닥 예약의 적용 범위·임시 정책·취소로 대응 가능)로 구성합니다. 닫기 버튼은 두지 않고, 설정을 다시 ON으로 바꾸면 한 줄 안내로 바뀝니다. 톤은 오류가 아닌 주의 사항으로 처리합니다.',
    developerNotes: [
  ...
  },
  {
    ...
    after: '연동 병원이 자동 확정을 ON에서 OFF로 바꾸는 순간 확인 모달을 표시합니다. `카카오톡 예약하기로 받는 예약은 계속 자동으로 확정됩니다`를 제목으로, 굿닥 예약의 적용 범위와 임시 정책이라는 사실을 본문 두 줄로 안내합니다. 진행 버튼은 `자동 확정 끄기`입니다. 취소하면 설정은 ON 상태로 유지되고, 확인해야 OFF가 적용됩니다.',
    developerNotes: [
