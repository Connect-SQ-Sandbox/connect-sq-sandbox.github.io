...
| 화면 | 소스 | 산출물 | 설명 |
|---|---|---|---|
| **예약 신청 내역 · 독감 무료접종형 진료정보** (신규) | `pages/connect/appt-free-vaccine/index.page.tsx` | `out/appt-free-vaccine.html` | `admin-nonpay-aug`(v45)에서 분기. 환자가 무료 백신 대상자(임신부·어린이·어르신)로 신청해 제품을 특정하지 않은 예약을 병원 쪽 **예약 신청 내역·상세**가 어떻게 보여줘야 하는지 정리. 진료정보에 **유형 구분자(상품형/무료접종형)** 신설, 식별자는 **무료 백신 대상자 유형**. 목록은 금액 자리에 `무료접종 · {대상자}`, 상세 카드는 분류 뱃지 · "상담 후 결정" · 대상자 행(유형 + 신청 시점 기준 문구 스냅샷) · 상태 1줄 · 고지 문구, 옵션·금액·소개·썸네일 없음. mock은 독감 시즌(2026-10)으로 이동하고 무료접종형 3건 추가. `?spec=1` **스펙 보기**로 목록 열·상세 카드·대상자 행의 정책(As-is/To-be/결정/미결)을 패널로 확인. 짝 화면은 **일괄 등록 보완안**(`out/treatment-create-plus.html`, 독감 9종 일괄 등록) — 무료접종은 등록 상품과 무관한 별도 유형이라 등록 화면엔 무료 입력이 없다. 참여기관 설정·필터·기준 문구 어드민은 범위 밖(헤더 참고). |
| **진료항목 어드민 · 분류 선택 트리판** (신규) | 소스 없음 — `out/treatment-create-plus.html`(v2.5) 카피에 패치 | `out/treatment-create-tree.html` | 보완안의 **분기**. 보완안은 그대로 유지합니다. 분류 선택을 모달에서 꺼내 폼 안에 **3열 캐스케이드 트리**로 놓았습니다 — 대분류 → 중분류 → 소분류, 이전 열을 고르면 다음 열이 채워집니다. 최초 상태는 **추천 항목**(대분류 열 첫 행)이고 고르면 우측 두 칸이 합쳐져 독감 9종 목록이 됩니다. **기타는 대분류에서 제거하고 중·소분류에만** 둡니다(중분류 기타를 고르면 소분류 열에 기타 하나). 카운트 기준은 모달과 동일 — 대분류는 중분류 수 + 1(기타 포함), 중분류는 소분류 수(기타 제외). 트리 위에 **검색**이 있습니다 — 결과를 레이어로 띄우고 `대분류 › 중분류` 그룹 아래 소분류를 나열하며, 고르면 값이 잡히고 하단 트리의 세 열도 그 경로로 이동합니다. 본문 추천 칩은 대분류 열의 추천 항목과 중복이라 빠졌습니다. **우측 하단 [스펙 보기]** 토글(`?spec=1`)을 켜면 분류·검색·트리·항목명·가격 다섯 영역에 ⓘ 핀이 붙고, 누르면 그 영역의 정책이 패널로 뜹니다 — 피그마 디스크립션을 프로토타입 안으로 옮겨 화면과 정책이 어긋날 수 없게 하는 핸드오프 실험입니다. 파일 맨 끝 독립 블록(`[T20]`)이라 그 블록만 지우면 완전히 제거됩니다. |
| **9월 진료항목 어드민 개편 · 일괄 등록 보완안** (신규) | 소스 없음 — `out/treatment-create.html`(PO 시안 9/7 배포본) 카피에 패치 | `out/treatment-create-plus.html` | PO 시안의 **보완 카피**(원본 무수정). **① 스키마 전환 수정** — 분류 변경 확정 핸들러가 `lockedSchemaName`을 항상 비우던 것을 새 소분류명으로 세팅. 마운자로·위고비 선택 시 조제 경로(원내/원외)+용량별 약제비·조제비·총액 자동계산이 실제로 뜹니다. 부작용(입력 없이도 초기화 모달이 뜨던 것) 보정, `vl` 사전에 대상포진 제품 3종 추가(원본 키가 중분류명이라 분류 트리에서 도달 불가였음). 상세 상단 `목록` 버튼이 `history.back()`이라 동작하지 않던 것도 목록 전환으로 교체. 입력형태 비교 토글(표준/자유)은 숨김. **② 일괄 등록 신설** — 목록 상단 배너에서 독감 9종을 한 화면에 펼쳐 취급 체크·접종료만 입력. 추천 6종/그 외 3종 분리, 기등록 프리필, 부분 실패 시 성공분만 저장하고 실패 행은 사유와 함께 유지. 일괄 시트는 vanilla 주입이라 목록과 동기화되지 않습니다(원본이 minified 번들). |
