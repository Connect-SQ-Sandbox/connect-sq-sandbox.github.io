...
export const ADMIN_NONPAY_AUG_CHANGES: PolicyChange[] = [
   ...
   * 설정하는 곳·예외가 정해지는 곳·결과가 나타나는 곳이 서로 달라서, 안내를 세 지점에 나눠 배치한다. */
  {
    ...
    after: '자동 확정이 ON인 연동 병원에는 설명 아래에 `카카오톡 예약하기로 받는 예약은 이 설정과 관계없이 자동으로 확정됩니다.`를 한 줄로 표시합니다. 설정 항목 자체가 적용 범위를 말하도록 해, 나중에 설정을 끌 때 처음 알게 되는 상황을 막습니다. OFF일 때는 같은 내용을 더 자세히 말하는 안내 블록이 대신 표시되므로 이 한 줄은 숨깁니다.',
  ...
  },
  {
    ...
    after: '연동 병원이면서 자동 확정이 OFF일 때, `예약 자동 확정` 설정 박스 안에 안내 블록을 표시합니다. 제목·설명·토글 한 줄 아래에 전체 폭으로 붙습니다. 제목과 세 개의 안내(임시 정책·굿닥 예약의 적용 범위·취소로 대응 가능)로 구성합니다. 제목이 설정과 반대되는 사실을 알리므로 `왜`에 먼저 답하고 대처 방법을 끝에 둡니다. 닫기 버튼은 두지 않고, 설정을 다시 ON으로 바꾸면 한 줄 안내로 바뀝니다. 톤은 오류가 아닌 주의 사항으로 처리합니다.',
    developerNotes: [
  ...
  },
  {
    ...
    after: '연동 병원이 자동 확정을 ON에서 OFF로 바꾸는 순간 확인 모달을 표시합니다. `카카오톡 예약하기로 받는 예약은 계속 자동으로 확정됩니다`를 제목으로, 임시 정책이라는 사실과 굿닥 예약의 적용 범위를 본문 두 줄로 안내합니다. 진행 버튼은 `자동 확정 끄기`입니다. 취소하면 설정은 ON 상태로 유지되고, 확인해야 OFF가 적용됩니다.',
    developerNotes: [
  ...
  },
  {
    ...
    after: '병원이 수동 확정으로 운영 중일 때, `카카오톡 예약하기에서도 보이기` 토글의 설명 영역에 `카카오톡 예약하기로 받는 예약은 자동으로 확정됩니다.`를 한 줄로 표시합니다. 별도 안내 블록이 아니라 토글 설명의 일부로 배치해 `이 토글을 켜면 어떻게 되는지`로 읽히게 합니다.',
    developerNotes: [
