...
/**
 * ┌─ 프로토타입 컨텍스트 ───────────────────────────────────
 * 이름     : appt-free-vaccine — 예약 신청 내역 · 독감 무료접종형(상품 미특정) 진료정보
 *            (admin-nonpay-aug v45에서 분기. 예약 신청 내역·상세의 진료정보 영역에 "무료접종형" 유형을 추가)
 * 상태     : 현행(active)   버전: v1  최종수정: 2026-09-14
 * PRD      : 미발행 — 세화님 정책 초안 `진료항목_예약신청내역_무료접종형_정보구조.md`(2026-09-14, 로컬) 기준
 *            유저향 흐름(무료 백신 대상자 선택 → 제품 선택 건너뜀 → "상담 후 결정"으로 신청)은 타 담당자 범위
 * 짝 화면  : 진료항목 어드민 · 일괄 등록 보완안 = out/treatment-create-plus.html (독감 9종 일괄 등록). 이 정책은 그 등록 흐름과
 *            함께 추가된다. 단, 무료접종은 등록 상품과 무관한 별도 유형이라 등록 화면엔 무료 관련 입력이 없다.
 * 배포URL  : https://connect-sq-sandbox.github.io/out/appt-free-vaccine.html
 * 관련 CSS : connectRegister.css + connectAdminNonpayAug.css + apptFreeVaccine.css
 * 기술제약 : react-only · plain CSS · mock · 네트워크 0
 *
 * 화면구성 : ① 예약 신청 내역(기본 진입) — 목록 진료항목 열에 무료접종형 표기  ② 예약 상세 — 진료 정보 카드 무료접종형 분기
 *            ③ [스펙 보기] 토글(`?spec=1`) — 목록 열·상세 카드·대상자 행에 ⓘ 핀, 누르면 정책 패널
 *            (진료항목·설정·운영시간 화면은 분기 원본 그대로, 이번 범위 아님)
 *
 * 핵심 결정 (why):
 *   [확정·초안] 진료정보는 신청 시점 스냅샷(As-is 동일). 무료접종형도 같다.
 *   [확정·초안] 진료정보에 유형 구분자 신설 — 상품형(As-is) / 무료접종형(신규). 데이터는 Appt.freeVaccine 유무.
 *   [확정·초안] 무료접종형의 식별자 = 무료 백신 대상자 유형(임신부·어린이·어르신).
 *              항목명·제품 미특정·금액 "무료"는 그 유형이 선택됐을 때의 파생 표현이다.
 *   [확정·초안] 무료접종형은 병원이 등록한 독감 상품과 무관 — 상품 0개여도 발생. 그래서 상품 옵션이 아니라 별도 유형.
 *   [확정·초안] 대상자 기준 문구(출생 범위)는 굿닥 운영값 → 신청 시점 값을 스냅샷(매년 바뀜). 임신부는 기준 문구 없음.
 *   [확정·초안] 병원향 항목명은 "독감 무료접종". 환자 화면의 "상담 후 결정"을 그대로 쓰지 않는다 —
 *              어드민에서 그 말은 가격 유형이라 항목명 자리에 있으면 "가격을 상담 후 정하는 상품"으로 읽힌다.
 *   [확정·초안] 목록 진료항목 열 — 1행 "독감 무료접종", 2행 금액 자리에 "{대상자 유형} · 무료".
 *   [확정·초안] 상세 진료 정보 카드 — 상품형 구조 그대로(최소 변경). 분류 뱃지(독감백신) · 항목명(독감 무료접종) ·
 *              옵션 행 자리에 대상자 행 1개("{유형} ({기준 문구})" / "무료") · 예상 결제 금액 "무료" ·
 *              고지 문구(대상 여부는 병원 확인, 미대상 시 유료 안내). 소개 문구·썸네일은 없다(원본 상품이 없음).
 *              ?card=custom 은 대상자 박스 + 상태 1줄("국가 무료접종 대상자로 신청됨") 전용 레이아웃 — 비교용이고 채택안이 아니다.
 *   [확정·초안] 대상 적합 여부는 굿닥이 판정하지 않는다. 병원이 방문자 생년월일과 기준 문구를 눈으로 대조.
 *   [유지·자체] mock 날짜를 독감 시즌(2026-10)으로 3개월 이동. 무료접종형 3건(어르신·어린이[보호자 예약]·임신부) 추가.
 *   [유지·자체] 스펙 보기 오버레이는 treatment-create-tree [T20] 방식을 React 컴포넌트로 옮긴 것. 기본 off.
 *
 * 미결 4건 정리 (2026-09-14 세화님 결정):
 *   [확정·PO] 참여기관 설정은 어드민에 두지 않는다. 공공기관(위탁의료기관) 데이터로 참여 여부를 판정.
 *              가설 = 참여기관이면 무료접종 예약을 굳이 거부하는 병원은 없다. → 이 프로토타입엔 설정 UI 없음.
 *   [확정·PO] 목록 필터 "무료접종만 보기"는 오버스펙 — 제외. 독감 상품 등록·예약 트래픽이 크지 않을 전망.
 *   [확정·PO] 유료 전환 시 내역 수정 없음(스냅샷 유지, 고지 문구로 커버). 유료 접종 가격은 이미 상품에 등록돼 있어
 *              병원이 현장에서 그 가격으로 안내한다.
 *   [보류]    대상자 기준 문구 운영값 관리 위치 — 제안: goodoc-api 측 "국가예방접종 절기 설정" 1레코드(절기·대상군별
 *              라벨·기준 문구·시작일), 어드민 UI 없이 연 1회 운영 갱신, 예약 생성 시 라벨+문구 복사. 여기선 상수로 대체.
 *   [제안]    공공데이터 판정의 빈틈 2가지(사업별 위탁 구분 · 국가 물량 소진)는 병원 취소 사유 템플릿 보강으로 흡수 검토.
 *
 * 변경 이력:
 *   v1  2026-09-14 — admin-nonpay-aug v45 분기. 무료접종형 유형·mock·목록 셀·상세 카드·스펙 보기 추가. 기본 진입 = 예약 신청 내역.
 *   (분기 원본 이력은 pages/connect/admin-nonpay-aug/index.page.tsx 헤더 참고)
 * └──────────────────────────────────────────────────────
 */
...
const MOCK_TODAY = '2026-10-15';
type ApptAdditionalInfo = { name: string; value?: string; values?: string[] };
/* ---------- [신규] 무료접종형 진료정보 ----------
 * 식별자 = 무료 백신 대상자 유형. 기준 문구는 굿닥 운영값을 신청 시점에 스냅샷한다(매년 갱신).
...
type Appt = {
  ...
  /** [신규] 있으면 진료정보 유형 = 무료접종형(상품·옵션·금액 없음). 없으면 상품형(As-is). */
  freeVaccine?: FreeVaccineInfo;
};
let PID = 5000;
const INITIAL_APPTS: Appt[] = [
  // [신규] 무료접종형 — 어르신. 본인 예약. 상품·옵션·금액 없음.
  ...
  { treatmentItemApptId: 202, status: APPT_STATUS.REQUESTED, deviceType: 1, visitDate: '20261018', visitKoTime: '11:30', createdAt: '2026-10-14T08:40:00',
  ...
  // [신규] 무료접종형 — 어린이. 보호자 예약(예약자 ≠ 방문자). 요청사항 있음.
  { treatmentItemApptId: 302, status: APPT_STATUS.REQUESTED, deviceType: 1, visitDate: '20261017', visitKoTime: '15:30', createdAt: '2026-10-13T11:05:00',
    treatmentItem: { name: '독감백신', master1Id: null }, prices: [], freeVaccine: freeVaccine('CHILD'),
    visitorName: '이도현', visitorPhone: '01077778888', visitorBirth: 20230415, visitorRrn7: 3,
    reserverName: '이수민', reserverPhone: '01077778888', reserverMemo: '첫 접종이라 2회 접종 대상인지 확인 부탁드려요.' },
  { treatmentItemApptId: 203, status: APPT_STATUS.CONFIRMED, deviceType: 2, visitDate: '20261019', visitKoTime: '14:00', createdAt: '2026-10-14T10:02:00', statusChangedAt: '2026-10-14T10:31:00',
  ...
  { treatmentItemApptId: 204, status: APPT_STATUS.CONFIRMED, deviceType: 1, visitDate: '20261020', visitKoTime: '16:30', createdAt: '2026-10-13T17:20:00', statusChangedAt: '2026-10-13T18:02:00',
  ...
  // [신규] 무료접종형 — 임신부(기준 문구 없음). 예약확정 상태.
  { treatmentItemApptId: 303, status: APPT_STATUS.CONFIRMED, deviceType: 1, visitDate: '20261019', visitKoTime: '09:30', createdAt: '2026-10-12T19:40:00', statusChangedAt: '2026-10-12T19:41:00',
  ...
  { treatmentItemApptId: 206, status: APPT_STATUS.CANCELED_BY_PATIENT, deviceType: 1, visitDate: '20261006', visitKoTime: '10:00', createdAt: '2026-10-05T20:10:00', statusChangedAt: '2026-10-06T08:12:00',
  ...
  { treatmentItemApptId: 207, status: APPT_STATUS.CANCELED_BY_HOSPITAL, deviceType: 2, visitDate: '20261006', visitKoTime: '18:00', createdAt: '2026-10-06T09:30:00', statusChangedAt: '2026-10-06T12:30:00',
  ...
  { treatmentItemApptId: 208, status: APPT_STATUS.REQUESTED, deviceType: 2, visitDate: '20260918', visitKoTime: '04:30', createdAt: '2026-09-17T18:03:00',
  ...
  { treatmentItemApptId: 209, status: APPT_STATUS.CONFIRMED, deviceType: 1, visitDate: '20260920', visitKoTime: '11:00', createdAt: '2026-09-19T10:00:00', statusChangedAt: '2026-09-19T10:20:00',
...
/** [신규] 목록 진료항목 열 — 무료접종형은 1행 "독감 무료접종", 2행 금액 자리에 "{대상자} · 무료". */
...
function ApptScreen({ appts, setAppts, hospitalLinked, failNextSync, consumeFailure, focusAdditionalToken, showToast, spec }: {
  spec: SpecCtl;
  appts: Appt[];
  ...
  const rows = useMemo(() => {
    ...
    if (applied?.text) {
      ...
      r = r.filter((a) => {
        ...
        return (a.treatmentItem.alias || a.treatmentItem.name || '').includes(q) || a.prices.some((p) => p.title.includes(q)) || freeSearchText(a).includes(q);
      });
    }
    return r;
        ...
        <div className="apx-table-scroll">
          <div className={`apx-table${hospitalLinked ? ' has-channel' : ''}`}>
            <div className="apx-tr apx-th">
              ...
              <span><SpecHost id="list-col" inline spec={spec}>진료항목</SpecHost></span>
              <span>방문자</span>
              <span>예약자</span>
              <span>요청사항</span>
            </div>
            {pageRows.length === 0 ? (
              ...
              <div key={a.treatmentItemApptId} className="apx-tr apx-row" onClick={() => setDetailId(a.treatmentItemApptId)}>
                ...
                <span><TwoLineCell primary={itemCellPrimary(a)} sub={itemCellSub(a)} /></span>
                  ...
                  {detail.freeVaccine && !CARD_CUSTOM ? (
                    ...
                    <>
                      ...
                      <div className="apx-pad apx-treat apx-treat-free">
                        ...
                        <SpecHost id="target-row" tbd spec={spec}>
                          <div className="apx-free-target">
                            <span className="apx-free-target-label">무료 백신 대상자</span>
                            <span className="apx-free-target-val">
                              {detail.freeVaccine.targetLabel}
                              {detail.freeVaccine.criteria && <em className="apx-free-target-crit">({detail.freeVaccine.criteria})</em>}
                            </span>
                          </div>
                        </SpecHost>
                        <div className="apx-divider" />
                        <div className="apx-free-status"><span className="apx-free-status-ic"><InfoIc /></span>{FREE_STATUS_LINE}</div>
                      </div>
                      <div className="apx-card-footer apx-treat-footer">{FREE_FOOTER}</div>
                    </>
                  ) : (
...
/* ============================================================ */
/* ============================ [SX] 스펙 보기 오버레이 ============================
 * treatment-create-tree [T20]의 핸드오프 실험을 React로 옮긴 것. 기본 off(`?spec=1` 또는 우측 상단 토글).
 * 화면과 정책이 한 파일에 있으면 둘이 어긋날 수 없다는 발상. 정책 원문은 세화님 초안 md(2026-09-14).
 * 이 블록(SpecCtl·SPEC·SpecHost·SpecFab·SpecPanel)과 apptFreeVaccine.css의 .sx-* 만 지우면 완전히 제거된다. */
type SpecCtl = { on: boolean; id: string | null; open: (id: string) => void };
type SpecEntry = { title: string; where: string; tbd?: boolean; asis?: string[]; tobe: string[]; decided?: string[]; open?: string[]; note?: string };
const SPEC: Record<string, SpecEntry> = {
  'list-col': {
    title: '목록 · 진료항목 열',
    ...
    tobe: [
      ...
      '상품형 행은 As-is 그대로',
      '검색(진료항목명)은 "독감", "무료접종", "국가", 대상자 유형으로도 걸린다',
      '무료접종형만 모아 보는 필터는 없다'
    ]
  },
  'detail-card': {
...
function SpecHost({ id, tbd, inline, spec, children }: { id: string; tbd?: boolean; inline?: boolean; spec: SpecCtl; children: React.ReactNode }) {
  if (!spec.on) return <>{children}</>;
  const act = spec.id === id;
  return (
    <span className={`sx-host${inline ? ' inline' : ' block'}${act ? ' act' : ''}${tbd ? ' tbd' : ''}`}>
      {children}
      <button type="button" className={`sx-pin${tbd ? ' tbd' : ''}${act ? ' act' : ''}`} title={SPEC[id]?.title} aria-label={`스펙: ${SPEC[id]?.title ?? id}`}
        onClick={(e) => { e.stopPropagation(); spec.open(id); }}>{tbd ? '?' : 'i'}</button>
    </span>
  );
}
function SpecFab({ on, onToggle }: { on: boolean; onToggle: () => void }) {
  return (
    <button type="button" className={`sx-fab${on ? ' on' : ''}`} onClick={onToggle} aria-pressed={on}>
      <span className="sx-dot" />{on ? '스펙 보기 켬' : '스펙 보기'}
    </button>
  );
}
function SpecPanel({ entry, onClose }: { entry: SpecEntry; onClose: () => void }) {
  ...
  return (
    <aside className="sx-panel" role="dialog" aria-label={entry.title} onClick={(e) => e.stopPropagation()}>
      <div className="sx-panel-head">
        ...
        <button type="button" className="sx-panel-close" onClick={onClose} aria-label="닫기"><CloseIcon /></button>
      </div>
      <div className="sx-panel-body">
        {entry.asis && (<section><h4>As-is</h4><ul>{entry.asis.map((t, i) => <li key={i}>{t}</li>)}</ul></section>)}
        <section><h4>{entry.asis ? 'To-be (달라지는 것만)' : '정의'}</h4><ul>{entry.tobe.map((t, i) => <li key={i}>{t}</li>)}</ul></section>
        {entry.decided && (<section className="decided"><h4>결정 · 2026-09-14</h4><ul>{entry.decided.map((t, i) => <li key={i}>{t}</li>)}</ul></section>)}
        {entry.open && (<section className="tbd"><h4>미결</h4><ul>{entry.open.map((t, i) => <li key={i}>{t}</li>)}</ul></section>)}
        {entry.note && <p className="sx-panel-note">{entry.note}</p>}
      </div>
    </aside>
  );
}
...
function TiKakao() {
  const [page, setPage] = useState<Page>('appt'); // [분기] 기본 진입 = 예약 신청 내역
  const [specOn, setSpecOn] = useState<boolean>(() => { try { return new URLSearchParams(window.location.search).get('spec') === '1'; } catch { return false; } });
  const [specId, setSpecId] = useState<string | null>(null);
  const spec: SpecCtl = { on: specOn, id: specId, open: (id) => setSpecId((cur) => (cur === id ? null : id)) };
      ...
      <div className="cn-screen">
        ...
        <div className="cn-body">
          ...
          <main className="cn-main rg-main tk-main">
            {/* ========================= 예약 신청 내역 ========================= */}
            {page === 'appt' && <ApptScreen appts={appts} setAppts={setAppts} hospitalLinked={hospitalLinked} failNextSync={failNextSync} consumeFailure={() => setFailNextSync(false)} focusAdditionalToken={focusAdditionalToken} showToast={showToast} spec={spec} />}
        ...
        <SpecFab on={specOn} onToggle={() => { setSpecOn((v) => !v); setSpecId(null); }} />
        {specOn && specId && SPEC[specId] && <SpecPanel entry={SPEC[specId]} onClose={() => setSpecId(null)} />}
        <ChangeDrawer
