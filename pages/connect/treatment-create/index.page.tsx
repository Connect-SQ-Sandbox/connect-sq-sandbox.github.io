...
type PolicyChange = any; type PrototypeView = any;
const ChangeDrawer = (_props: any) => null;
const POLICY_SOURCES: any[] = []; const ADMIN_NONPAY_AUG_CHANGES: any[] = [];

/** TIM-1 · 2026-09-07 published draft preview; no production service changes. */
/* ============================ 진료 항목 타입 & mock ============================ */
...
type Item = {
  ...
  customLeaf?: boolean; // Local prototype only: custom classification, never a master mapping.
...
/* 답변 항목(선택지) 중복 검증
 * 카카오는 이용자가 고른 답변을 문구(value) 그대로 저장·전달한다. 같은 문구가 두 개면
 * 이용자 화면에서 두 항목을 구분할 수 없고(중복 선택처럼 보임), 예약 상세의 답변으로도
...
/* =========================================================================
 * 굿닥 표준 진료 항목 (분류 트리) — C안 신설. 2026-08-14 마스터 전량.
 * ======================================================================= */
...
/** 굿닥 표준 진료 항목 — 2026-08-14 실제 마스터 전량(대 15 / 중 69 / 소 187) */
 ...
 * 항목별 가격 스키마 — C안 확장 제안
 *
 * 왜 : 공통 단위 폼(PO 6.3)은 모든 진료 항목에 자유 입력(단위·수량 직접입력)을 요구한다.
 ...
 *
 * 개념 : 진료 항목을 **표준 상품(Canonical Product)** 과 **자유 상품(Custom Product)** 으로 나눈다.
...
const PRICE_SCHEMAS: Record<string, PriceSchema> = {
  ...
  /* 제품명은 **굿닥 표준 진료 항목의 소분류명**을 그대로 쓴다 — 나중에 master3Id 로 걸어야 하므로
  ...
  '대상포진 백신': {
    ...
    source: '표준 진료 항목 소분류 3종 · 제품별 접종 프로토콜'
  }
};
 ...
 *
 *  · 한 진료 항목 안에서는 **공통 단위 하나**를 고르고, 옵션별로 수량과 가격을 넣는다.
...
const ALIAS_MAX = 50;         // [서비스] 진료 항목 노출명 — 현재 50자
const INTRO_MAX = 50;         // [서비스] 한 줄 소개 — 현재 50자
const KEYWORD_MAX = 20;       // [서비스] 진료 항목 키워드 개수 — 현재 20개
...
const APPT_STATUS = {
  ...
  CANCELED_BY_PATIENT: 'F02', // 본인취소(이용자취소)
...
const APPT_STATUS_LABEL: Record<string, string> = {
  ...
  [APPT_STATUS.CANCELED_BY_PATIENT]: '이용자취소',
...
const getClosedReason = (status?: string, tab?: TabValue): string | undefined => {
  ...
  if (status === APPT_STATUS.CANCELED_BY_PATIENT) return '이용자 취소';
  return undefined;
};
...
const SEARCH_TYPE_LABEL: Record<SearchType, string> = {
  [SEARCH_TYPE.PATIENT_NAME]: '이용자명', [SEARCH_TYPE.PHONE]: '연락처', [SEARCH_TYPE.TREATMENT_ITEM_NAME]: '진료 항목명'
};
...
const SEARCH_TYPE_PLACEHOLDER: Record<SearchType, string> = {
  [SEARCH_TYPE.PATIENT_NAME]: '이용자명을 입력해 주세요.',
  [SEARCH_TYPE.PHONE]: '연락처를 입력해 주세요.',
  [SEARCH_TYPE.TREATMENT_ITEM_NAME]: '진료 항목 또는 가격명을 입력해 주세요.'
};
...
const CANCEL_REASON_TEMPLATES: { id: string; label: string }[] = [
  ...
  { id: 'treatment_item_check', label: '진료 항목 확인 필요' },
  { id: 'patient_info_check', label: '이용자 정보 확인 필요' },
...
/** [실제] 표준 진료 항목(마스터 매핑)인지 — 직접입력이면 항목명 뱃지 미노출. */
 ...
 *   ① 운영 설정 — 내 설정이 어디까지 적용되는가  ② 진료 항목 상세 — 이 항목을 켜면 어떻게 되는가
...
/** ② 진료 항목 상세 — 카카오 노출 토글 설명. 한 줄에 들어가도록 A만 쓴다(B는 설정 화면 몫). */
    ...
    <nav className="cn-nav">
      <div className="cn-nav-scroll">
        <div className="cn-nav-group">
          ...
          <N label="진료 항목" sub active={page === 'items'} onClick={() => onNav('items')} />
    ...
    return (
      <div className="tc-cp">
        <div className="tc-cp-note">
          취급하는 제품에만 접종료를 넣어 주세요. <b>가격을 넣은 제품만</b> 이용자에게 노출됩니다.
...
// Display name is derived, never written over the independent manual title / quantity / unit fields.
const priceOptionName = (p:Price,unit?:string) => unit&&unit!==NO_UNIT ? (p.qty&&Number(p.qty)>0?`${Number(p.qty)}${unit}`:'수량 입력 전') : p.title;
function PriceRow({ p, unit, onChange, onDelete, onDragStart, onDrop, titleErr, amountErr }: { p: Price; unit: string; onChange: (u: Partial<Price>) => void; onDelete: () => void; onDragStart: () => void; onDrop: () => void; titleErr?: string; amountErr?: string }) {
  ...
  return (
    <div className={'rg-price-row tk-price-row'+(p.preset?' sep-preset-row':'')} draggable={!p.preset} onDragStart={onDragStart} onDragOver={(e) => e.preventDefault()} onDrop={onDrop}>
      {!p.preset&&<div className="rg-drag" aria-label="순서 변경 핸들"><DragHandle /></div>}
      <div className="rg-price-fields">
        {p.preset?<div className="sep-preset-title">기본 가격 · 1회 </div>:unit!==NO_UNIT?<div className="sep-preset-title">{priceOptionName(p,unit)} <span>수량과 단위로 표시돼요.</span></div>:<input aria-label="가격 옵션명" className={`rg-input${titleErr ? ' error' : ''}`} placeholder={`가격명을 입력해 주세요. (15자 권장, 최대 ${PRICE_NAME_MAX}자)`} maxLength={PRICE_NAME_MAX} value={p.title} onChange={(e) => onChange({ title: e.target.value })} />}
        {titleErr && <p className="rg-error">{titleErr}</p>}
        {!p.preset&&<input className="rg-input" placeholder={`가격 설명을 입력해 주세요. (선택사항, 최대 ${PRICE_DESC_MAX}자)`} maxLength={PRICE_DESC_MAX} value={p.content} onChange={(e) => onChange({ content: e.target.value })} />}
        <div className="rg-price-entry">
          {/* 수량 — 공통 단위가 '단위 없음'이면 비활성 + 빈값 유지 (PO 6.3) */}
          {!p.preset&&<div className="tc-qty">
            <input
              ...
              aria-label="수량"
              inputMode={VOLUME_UNITS.includes(unit)?'decimal':'numeric'}
              onChange={(e) => { const value=e.target.value; if((VOLUME_UNITS.includes(unit)?/^\d*\.?\d*$/:/^\d*$/).test(value))onChange({qty:value}); }}
            />
          ...
          {p.type === 'consult' && <span className="rg-help">상담 후 가격 안내</span>}
        </div>
        {amountErr && <p className="rg-error">{amountErr}</p>}
      </div>
      {!p.preset&&<button className="rg-price-del" onClick={onDelete} aria-label="삭제"><CloseIcon /></button>}
    </div>
  );
}
function GoodocPreview({ d, lockedSchemaName = '' }: { d: Item; lockedSchemaName?: string }) {
  const categoryName = categoryDisplayName(d);
  const title = d.alias.trim();
  ...
  const schema = PRICE_SCHEMAS[lockedSchemaName];
    ...
    <div className="rg-preview tk-preview">
      <div className="tk-pv-scroll">
        ...
        <div className="tk-pv-body">
          <div className="sep-preview-category" data-testid="preview-category">{d.cat1==='기타'?'기타':categoryName || '진료 항목 분류'}</div>
          <div className={`tk-pv-title${title ? '' : ' ph'}`}>{title || '진료 항목을 입력해 주세요.'}</div>
            ...
            : schema && d.rx
              ...
              : d.prices.map((p) => (
                  <div key={p.id} className="tk-pv-price-row">
                    <span className={`tk-pv-price-name${priceOptionName(p,d.unit) ? '' : ' ph'}`}>{priceOptionName(p,d.unit) || '가격명'}</span>
                    <span className="tk-pv-price-val">{p.type === 'discount' && Number(p.original)>0 && <span className="tk-pv-strike">{won(p.original)}</span>}{p.type!=='consult'&&!(Number(p.type==='discount'?p.sale:p.amount)>0)?'가격 입력 전':priceLine(p)}</span>
                  </div>
                ))}
  ...
  return (
    <>
      <div className="cn-header ap-header">
        ...
        <div className="ap-sub">등록한 진료 항목으로 예약 받은 내역을 관리합니다.</div>
      </div>
 ...
 *  API(usePatchApptUsed 등) → 로컬 state로 mock. 진료 항목 개수는 목록 mock과 연동. */
...
function SettingsScreen({ itemCount, visibleCount, operation, hospitalLinked, onGlobalChange, onSettingChange, showToast, onHours }: {
  ...
  const handleApptUsed = () => {
    if (!operation.apptUsed) {
      // [명세] OFF→ON: 노출 중(굿닥 노출 ON) 진료 항목이 0개면 예약을 받을 수 없어 차단
      if (visibleCount === 0) { showToast('노출 중인 진료 항목이 없어, 진료 예약을 받을 수 없습니다.'); return; }
      onGlobalChange(true);
  ...
  return (
    <>
      <div className="cn-header set-header">
        ...
        <div className="ap-sub">굿닥에 등록한 진료 항목으로 예약을 받을 수 있습니다.</div>
      </div>
      ...
      <div className="set-body" data-policy-id="gcp1-operation-settings">
        ...
        <SettingBox
          title="진료 예약 받기"
          subNode={<><span className="set-count">{itemCount}개의 진료 항목이</span><span className="set-count-rest"> 등록되어 있어요.</span></>}
          right={
        ...
        <section className="set-section">
          ...
          <SettingBox
            title="새 예약 알림 받기"
            subNode="굿닥과 카카오톡 예약하기의 새 예약이 발생하면, 이 PC에서 윈도우 알림을 받습니다. 이용자에게는 굿닥 알림을 추가로 보내지 않습니다."
        ...
        <div className="ap-dim" onClick={() => setStopOpen(false)}>
          <div className="ap-modal set-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="set-modal-body">
              그만 받기를 누르면 굿닥에서 진료 항목 노출과 예약 신청이 모두 중단돼요. 등록된 진료 항목은 그대로 유지되며, 다시 시작하면 바로 예약을 받을 수 있어요.
            </div>
...
/** Only category/name fields change. Never mutate unit, prices, rx, alias. */
const FIXED_CATEGORIES = ['일반 진료', '기타'];
const FLU_RECOMMENDED_IDS = [178,179,180,181,182,183];
const recommendedFlu = (item:Item) => !item.customLeaf && item.cat1==='예방접종' && item.cat2==='독감 백신' && TAXONOMY.find(a=>a.name===item.cat1)?.groups.find(b=>b.name===item.cat2)?.items.some(c=>c.name===item.name&&FLU_RECOMMENDED_IDS.includes(c.id));
const newFluPreset = ():Price => ({id:UID++,preset:true,title:'1회',content:'',qty:'1',type:'fixed',amount:'',original:'',sale:''});
const VOLUME_UNITS = ['cc']; // Prototype unit dictionary; physical DB mapping is not implemented.
const validQuantity=(value:string,unit:string)=>Number(value)>0&&(VOLUME_UNITS.includes(unit)?/^\d+(\.\d+)?$/.test(value):/^\d+$/.test(value));
const hasPriceInput = (item:Item) => item.prices.some(p=>p.amount||p.original||p.sale||p.content||p.type!=='fixed'||(!p.preset&&(p.qty||(p.title&&p.title!=='기본')))) || item.prices.length>1 || (!item.fluPreset&&!!item.unit&&item.unit!==NO_UNIT);
function categoryDisplayName(item: Item) {
  if (item.cat1==='일반 진료') return '일반 진료';
  if (item.name==='기타' && (item.cat1==='기타'||item.cat2==='기타'||TAXONOMY.find(a=>a.name===item.cat1)?.groups.some(b=>b.name===item.cat2))) return '기타';
  return TAXONOMY.find(a=>a.name===item.cat1)?.groups.find(b=>b.name===item.cat2)?.items.find(c=>c.name===item.name)?.name || '';
}

function PriceResetDialog({kind,onClose,onConfirm}:{kind:string;onClose:()=>void;onConfirm:()=>void}){
  const ref=useRef<HTMLDialogElement>(null);
  useEffect(()=>{ref.current?.showModal();},[]);
  return <dialog ref={ref} className="sep-dialog sep-convert" aria-labelledby="price-reset-title" onCancel={e=>{e.preventDefault();onClose();}}>
    <div className="sep-dialog-head"><h2 id="price-reset-title">{kind}를 변경하면 가격 정보를 다시 입력해야 해요.</h2></div>
    <div className="sep-convert-body"><p>입력한 가격 옵션이 모두 초기화됩니다.</p><p>진료 항목명과 소개는 유지됩니다.</p></div>
    <div className="sep-dialog-foot"><button autoFocus className="tc-cat-btn" onClick={onClose}>유지하기</button><button className="rg-btn-save" onClick={onConfirm}>초기화하고 변경</button></div>
  </dialog>;
}

function CategoryFirst({value, onChange}: {value: Item; onChange: (u: Partial<Item>) => void}) {
  const [open,setOpen] = useState(false);
  const [query,setQuery] = useState('');
  const [browseCategory,setBrowseCategory] = useState(''); // Empty = curated recommendations, not a category.
  const [expandedGroup,setExpandedGroup] = useState<number | null>(null);
  const opener = useRef<HTMLButtonElement>(null);
  const dialog = useRef<HTMLDialogElement>(null);
  const searchInput = useRef<HTMLInputElement>(null);
  const categoryNav = useRef<HTMLElement>(null);
  const leafPanel = useRef<HTMLElement>(null);
  const all = [...TAXONOMY.flatMap(a => a.groups.flatMap(b => b.items.map(c => ({c1:a.name,c2:b.name,c3:c.name,id:c.id})))),{c1:'일반 진료',c2:'일반 진료',c3:'일반 진료',id:-2}];
  const recommendations = FLU_RECOMMENDED_IDS.map(id => all.find(x=>x.id===id)!).filter(Boolean);
  const c1 = TAXONOMY.find(x=>x.name===value.cat1);
  const c2 = c1?.groups.find(x=>x.name===value.cat2);
  const c3 = c2?.items.find(x=>x.name===value.name)?.name ?? '';
  const fixedMajor = FIXED_CATEGORIES.includes(value.cat1) ? value.cat1 : '';
  const otherMiddle = value.cat2 === '기타';
  const patchCategory=(u:Partial<Item>)=>onChange({customLeaf:false,...u});
  const chooseOther=(major:string,middle:string)=>{patchCategory({cat1:major,cat2:middle,name:'기타'});close();};
  const q = query.trim().toLocaleLowerCase();
  const results = q ? all.filter(x => [x.c1,x.c2,x.c3].join(' ').toLocaleLowerCase().includes(q)) : recommendations;
  const activeCategory = TAXONOMY.find(x=>x.name===browseCategory);
  const browse=(name:string)=>{setBrowseCategory(name);setExpandedGroup(null);setQuery('');};
  const close=()=>{dialog.current?.close();setOpen(false);opener.current?.focus();};
  const choose=(x: typeof all[number])=>{patchCategory({cat1:x.c1,cat2:x.c2,name:x.c3});close();};
  useEffect(()=>{if(open){dialog.current?.showModal();searchInput.current?.focus();if(categoryNav.current)categoryNav.current.scrollTop=0;if(leafPanel.current)leafPanel.current.scrollTop=0;}},[open]);
  useEffect(()=>{if(leafPanel.current)leafPanel.current.scrollTop=0;},[browseCategory,q]);
  return <div className="rg-field sep-category">
    <FieldHead label="진료 항목 분류" helpers={['예약받을 진료 항목의 분류를 선택해 주세요. 굿닥 이용자가 찾고 비교하는 데 쓰여요.']} />
    <div className="sep-filters">
      <label>대분류<select className="rg-input" aria-label="대분류" value={fixedMajor||c1?.name||''} onChange={e=>patchCategory({cat1:e.target.value,cat2:FIXED_CATEGORIES.includes(e.target.value)?e.target.value:'',name:FIXED_CATEGORIES.includes(e.target.value)?e.target.value:''})}>
        <option value="" disabled>대분류 선택</option>{TAXONOMY.map(a=><option key={a.id} value={a.name}>{a.name}</option>)}{FIXED_CATEGORIES.map(name=><option key={name} value={name}>{name}</option>)}
      </select></label>
      <label>중분류<select className="rg-input" aria-label="중분류" disabled={!!fixedMajor||!c1} value={fixedMajor||(otherMiddle?'기타':c2?.name??'')} onChange={e=>patchCategory({cat2:e.target.value,name:e.target.value==='기타'?'기타':''})}>
        {fixedMajor?<option value={fixedMajor}>{fixedMajor}</option>:<><option value="" disabled>{c1?'중분류 선택':'대분류 먼저 선택'}</option>{c1?.groups.map(b=><option key={b.id} value={b.name}>{b.name}</option>)}<option value="기타">기타</option></>}
      </select></label>
      <label>소분류<select className="rg-input" aria-label="소분류" disabled={!!fixedMajor||otherMiddle||!c2} value={fixedMajor||(otherMiddle?'기타':value.name==='기타'?'기타':c3)} onChange={e=>patchCategory({name:e.target.value})}>
        {fixedMajor||otherMiddle?<option value={fixedMajor||'기타'}>{fixedMajor||'기타'}</option>:<><option value="" disabled>{c2?'소분류 선택':'중분류 먼저 선택'}</option>{c2?.items.map(c=><option key={c.id} value={c.name}>{c.name}</option>)}<option value="기타">기타</option></>}
      </select></label>
      <button type="button" className="tc-cat-btn sep-search-button" ref={opener} onClick={()=>{browse('');setOpen(true);}}><SearchIcon/>분류 찾아보기</button>

    </div>
    <div className="sep-recommend-head"><strong>추천 항목</strong></div>
    <div className="sep-chips">{recommendations.map(x=><button type="button" aria-pressed={!value.customLeaf&&c3===x.c3&&value.cat2===x.c2} className={'tc-unit-chip'+(!value.customLeaf&&c3===x.c3&&value.cat2===x.c2?' on':'')} key={x.id} onClick={()=>patchCategory({cat1:x.c1,cat2:x.c2,name:x.c3})}>{x.c2} <span>›</span> {x.c3}</button>)}</div>
    <dialog ref={dialog} className="sep-dialog" aria-labelledby="sep-dialog-title" onCancel={e=>{e.preventDefault();close();}} onClick={e=>{if(e.target===e.currentTarget)close();}}>
      <div className="sep-dialog-head"><div><h2 id="sep-dialog-title">진료 항목 분류 찾기</h2><p>항목을 선택하면 대·중·소분류가 함께 설정돼요.</p></div><button type="button" aria-label="분류 찾기 닫기" onClick={close}><CloseIcon/></button></div>
      <div className="sep-dialog-search"><SearchIcon/><input ref={searchInput} aria-label="분류 검색" placeholder="진료명이나 제품명 검색 (예: 독감, 가다실)" value={query} onChange={e=>setQuery(e.target.value)}/></div>
      <div className="tc-cols sep-browser">
        <div className="tc-col-left sep-browser-left">
          <div className="sep-recommend-row">
            <button type="button" className={'tc-col-c1'+(!browseCategory&&!q?' sel':'')} aria-pressed={!browseCategory&&!q} onClick={()=>browse('')}>추천 항목</button>
            <div className="sep-tooltip-wrap">
              <button type="button" className="sep-info-button" aria-label="추천 항목 안내" aria-describedby="sep-recommend-tip"><InfoIc/></button>
              <div role="tooltip" id="sep-recommend-tip">빠르게 등록할 수 있도록 굿닥이 미리 모아둔 항목이에요. 선택하면 대·중·소분류가 함께 설정돼요.<br/><span>이 시안에서는 독감 백신 6개를 먼저 안내해요.</span></div>
            </div>
          </div>
          <nav ref={categoryNav} className="sep-category-nav" aria-label="모달 대분류">
            {TAXONOMY.map(a=><button type="button" key={a.id} className={'tc-col-c1'+(browseCategory===a.name&&!q?' sel':'')} aria-pressed={browseCategory===a.name&&!q} onClick={()=>browse(a.name)}><span className="tc-col-c1-name">{a.name}</span><span className="tc-col-c1-count" aria-label={`중분류 ${a.groups.length+1}개`}>{a.groups.length+1}</span></button>)}
            {FIXED_CATEGORIES.map(name=><button key={name} type="button" className={'tc-col-c1'+(browseCategory===name&&!q?' sel':'')} aria-pressed={browseCategory===name&&!q} onClick={()=>browse(name)}><span className="tc-col-c1-name">{name}</span></button>)}
          </nav>
        </div>
        <section ref={leafPanel} className="tc-col-right" aria-label="소분류 탐색">
          <div className="tc-col-right-head" aria-live="polite">{q?`전체 분류 검색 · ${results.length}개`:FIXED_CATEGORIES.includes(browseCategory)?browseCategory:activeCategory?`${activeCategory.name} · 중분류를 펼쳐 소분류를 선택해 주세요`:'추천 항목 · 독감 백신'}</div>
          {!q&&FIXED_CATEGORIES.includes(browseCategory)?<div className="sep-results"><button type="button" onClick={()=>{patchCategory({cat1:browseCategory,cat2:browseCategory,name:browseCategory});close();}}><span className="sep-result-path">대·중·소분류 모두 {browseCategory}로 설정돼요.</span><strong>{browseCategory} › {browseCategory} › {browseCategory}</strong><span className="sep-pick">선택</span></button></div>:q||!activeCategory ? <div className="sep-results">{results.length?results.map(x=><button type="button" key={x.id} aria-label={`${x.c2} > ${x.c3} 선택`} aria-pressed={!value.customLeaf&&(fixedMajor||c3)===x.c3&&value.cat2===x.c2} onClick={()=>choose(x)}><span className="sep-result-path">{x.c1} › {x.c2}</span><strong>{x.c3}</strong><span className="sep-pick">{!value.customLeaf&&(fixedMajor||c3)===x.c3&&value.cat2===x.c2?'선택됨':'선택'}</span></button>):<div className="sep-empty">일치하는 분류가 없어요.<br/>다른 이름으로 검색하거나 왼쪽 분류를 살펴보세요.</div>}</div> : [...activeCategory.groups,{id:-1,name:'기타',items:[]}].map(group=>{
            const expanded=expandedGroup===group.id;
            return <div className="sep-group" key={group.id}>
              <button type="button" className={'tc-col-c2 sep-group-toggle'+(expanded?' expanded':'')} aria-expanded={expanded} aria-controls={`sep-group-${group.id}`} onClick={()=>setExpandedGroup(expanded?null:group.id)}>
                <span><span className="tc-col-c2-name">{group.name}</span><span className="tc-tree-hint">{group.items.length?`${group.items.length}개 소분류 · ${group.items.slice(0,2).map(x=>x.name).join(', ')}`:'중·소분류를 기타로 설정해요.'}</span></span><ChevronD/>
              </button>
              <div id={`sep-group-${group.id}`} className="sep-leaves" hidden={!expanded}>
                {group.items.map(leaf=>{const selected=!value.customLeaf&&value.cat1===activeCategory.name&&value.cat2===group.name&&c3===leaf.name;return <button type="button" className={'sep-leaf'+(selected?' selected':'')} aria-label={`${group.name} > ${leaf.name} 선택`} aria-pressed={selected} key={leaf.id} onClick={()=>choose({c1:activeCategory.name,c2:group.name,c3:leaf.name,id:leaf.id})}><span>{leaf.name}</span><span>{selected?'선택됨':'선택'}</span></button>;})}
                <button type="button" className="sep-leaf" aria-label={`${group.name} > 기타 선택`} onClick={()=>chooseOther(activeCategory.name,group.name)}><span>기타</span><span>선택</span></button>
              </div>
            </div>;
          })}
        </section>
      </div>
      <div className="sep-dialog-foot"><span>{value.name?<>현재 분류 <b>{value.cat1} › {value.cat2} › {value.name}</b></>:'소분류를 선택하면 분류가 적용돼요.'}</span><button className="tc-cat-btn" type="button" onClick={close}>닫기</button></div>
    </dialog>
  </div>;
}

function TiKakao() {
  const initialDraft = useMemo(() => mk({ id: UID++, name: '', cat1: '', cat2: '' }), []);
  const [lockedSchemaName, setLockedSchemaName] = useState('');
  const [pendingReset,setPendingReset] = useState<{patch:Partial<Item>;kind:'분류'|'단위'}|null>(null);
  ...
  const [screen, setScreen] = useState<'list' | 'form'>('form');
  const [selCat1, setSelCat1] = useState<string>('피부·미용');
  const [selId, setSelId] = useState<number | null>(null);
  const [d, setD] = useState<Item | null>(() => initialDraft);
  ...
  const [formBaseline, setFormBaseline] = useState(JSON.stringify(initialDraft));
  ...
  const applyReset=(u:Partial<Item>,kind:'분류'|'단위')=>{
    if(!d)return;
    const next={...d,...u,customLeaf:false};
    const flu=recommendedFlu(next);
    next.fluPreset=flu;
    next.unit=flu?'회':kind==='단위'?(u.unit||NO_UNIT):NO_UNIT;
    next.prices=flu?[newFluPreset()]:[{id:UID++,title:'기본',content:'',qty:'',type:'fixed',amount:'',original:'',sale:''}];
    next.rx={};
    setD(next);setLockedSchemaName('');setErrors({});setPendingReset(null);
  };
  const requestReset=(u:Partial<Item>,kind:'분류'|'단위')=>{
    if(!d)return;
    if(Object.entries(u).every(([k,v])=>d[k as keyof Item]===v))return;
    if(hasPriceInput(d)||Object.values(d.rx||{}).some(Boolean))setPendingReset({patch:u,kind});
    else applyReset(u,kind);
  };
  const changeCategory=(u:Partial<Item>)=>requestReset(u,'분류');
    ...
    return {
      ...
      kExtra: {
        ...
        productImages: prev.hasImage && prev.kExtra.productImages.length === 0
          ? [{ id: UID++, url: '', description: '굿닥 진료 항목 대표 이미지', fileName: '진료 항목_대표이미지.jpg' }]
  ...
  /* ── 브라우저 히스토리 연동 (뒤로/앞으로 양방향) ──────────
   * 화면 상태를 history.state 에 스냅샷으로 싣고 popstate 에서 **복원**한다.
   * 앞선 버전은 "뒤로가기 = 한 겹 닫기"였는데, 그러면 앞으로가기로 되돌릴 정보가 없다.
   *
   * 스냅샷에 담는 것 : page / screen / itemId / sheet
   *   · 이탈 확인 모달은 히스토리에 넣지 않는다(일시적 확인 단계).
   *   · 폼 복원은 items 에서 itemId 로 다시 클론한다.
   *
   * 주의 — 같은 폼 안에서 모달만 열고 닫을 때는 **드래프트를 다시 클론하지 않는다.**
   * 안 그러면 분류를 고르고 모달이 닫히는 순간 입력 중이던 값이 통째로 날아간다.
   * 그래서 itemId 가 같으면 sheet 플래그만 토글한다. */
  type ViewSnap = { page: Page; screen: Screen; itemId: number | null; sheet: boolean };

  const snap = useRef<ViewSnap>({ page: 'items', screen: 'form', itemId: null, sheet: false });
  const itemsRef = useRef(items);
  itemsRef.current = items;

  const pushView = (next: ViewSnap) => {
    snap.current = next;
    window.history.pushState(next, '');
  };

  const applyView = (v: ViewSnap) => {
    ...
    if (v.screen === 'form') {
      const same = cur.screen === 'form' && cur.itemId === v.itemId;
      if (!same) {
        const it = v.itemId != null ? itemsRef.current.find((x) => x.id === v.itemId) : null;
        const next = it ? cloneItem(it) : mk({ id: UID++, name: '', cat1: '', cat2: '' });
        setSelId(it ? it.id : null);
        setD(next); setLockedSchemaName(it?.name ?? '');
    ...
    setCatSheet(v.sheet);
    if (!v.sheet) { setSheetC1(null); setCatQuery(''); }
  };
  const applyViewRef = useRef(applyView);
  applyViewRef.current = applyView;

  useEffect(() => {
    window.history.replaceState(snap.current, '');
    const onPop = (e: PopStateEvent) => {
      const v = (e.state ?? { page: 'items', screen: 'list', itemId: null, sheet: false }) as ViewSnap;
  ...
  const nav = (p: Page) => {
    if (p === page) return;
    pushView({ page: p, screen: 'list', itemId: null, sheet: false });
    setPage(p);
    setScreen('list');
    setD(null);
    setSelId(null);
  ...
  const open = (it: Item) => { const next = cloneItem(it);
    /* 9/7 시안: 선택 이름의 빈 값은 유지하고 미리보기에서만 소분류명을 대체 표시한다. */
    setErrors({}); setLockedSchemaName(it.name); setSelId(it.id); setD(next); setFormBaseline(JSON.stringify(next)); setFormError(''); pushView({ page: 'items', screen: 'form', itemId: it.id, sheet: false }); setScreen('form'); };
  /* C안 — 신규 등록은 **분류를 비운 상태**로 시작한다.
   * 원래는 좌측에서 보고 있던 카테고리(selCat1/첫 그룹)를 그대로 물려줬는데, 그러면
   * 병원이 고르지도 않은 분류가 이미 박혀 있어 분류 필드가 무의미해진다. */
  const create = () => { setLockedSchemaName(''); const next = mk({ id: UID++, name: '', cat1: '', cat2: '' }); setErrors({}); setSelId(null); setD(next); setFormBaseline(JSON.stringify(next)); setFormError(''); pushView({ page: 'items', screen: 'form', itemId: null, sheet: false }); setScreen('form'); };
  const closeForm = () => { setScreen('list'); setD(null); setLeaveOpen(false); setFormError(''); setErrors({}); };
    const requestCloseForm = () => {
    if (d && JSON.stringify(d) !== formBaseline) setLeaveOpen(true);
    else window.history.back();
  };
  /** 이탈 확인 모달의 '확인' */
  const confirmLeave = () => { setLeaveOpen(false); window.history.back(); };
  // 저장 유효성 검증 — 카카오 상품 API required 필드 기준. 위반 필드별 메시지 맵을 반환(빈 객체면 통과).
  const collectErrors = (v: Item): Record<string, string> => {
    const e: Record<string, string> = {};
    /* 9/7 시안: 이름은 선택. 소분류도 없을 때만 사용할 이름을 요청한다. */
    const selectedLeaf = !!categoryDisplayName(v);
    if (!v.alias.trim()) e['alias'] = '진료 항목명을 입력해 주세요.';
    if (!v.cat1||!v.cat2||!selectedLeaf||v.customLeaf) e['category']='대·중·소분류를 모두 선택해 주세요.';
    /* 별도 시안: 직접 입력값은 표준 마스터로 승격하지 않는다. */
    ...
    const usingCanonical = !!PRICE_SCHEMAS[lockedSchemaName] && priceForm === 'canonical';
    if (!usingCanonical) v.prices.forEach((p) => {
      if(v.unit&&v.unit!==NO_UNIT&&!validQuantity(p.qty||'',v.unit))e[`price-${p.id}-title`]=VOLUME_UNITS.includes(v.unit)?'0보다 큰 용량을 입력해 주세요.':'0보다 큰 정수 수량을 입력해 주세요.';
      if ((!v.unit||v.unit===NO_UNIT)&&!p.title.trim()) e[`price-${p.id}-title`] = '가격명을 입력해 주세요.';
      if (p.type === 'fixed' && !(Number(p.amount)>0)) e[`price-${p.id}-amount`] = '0원보다 큰 가격을 입력해 주세요.';
      if (p.type === 'discount') {
        if (!(Number(p.original)>0) || !(Number(p.sale)>0)) e[`price-${p.id}-amount`] = '정상가와 판매가를 입력해 주세요.';
        else if (Number(p.sale) >= Number(p.original)) e[`price-${p.id}-amount`] = '정상가보다 낮은 가격을 입력해 주세요.';
        else if (Number(p.sale) < Math.floor(Number(p.original) * 0.51)) e[`price-${p.id}-amount`] = '최대 49%까지 할인할 수 있어요.';
      }
    });
    if(!usingCanonical){
      const seen=new Set<string>();
      v.prices.forEach(p=>{const key=JSON.stringify([priceOptionName(p,v.unit).trim().replace(/\s+/g,' ').toLowerCase(),v.unit===NO_UNIT?null:Number(p.qty),p.content.trim(),p.type,p.type==='fixed'?Number(p.amount):p.type==='discount'?[Number(p.original),Number(p.sale)]:null]);if(seen.has(key))e[`price-${p.id}-title`]='표시 이름·수량·내용·가격이 모두 같은 옵션이 있어요.';seen.add(key);});
    }
    if (v.kakaoOn) {
      v.prices.forEach((p) => {
        if (priceOptionName(p,v.unit).length > 25) e[`price-${p.id}-kakao`] = '카카오 가격명은 최대 25자예요.';
  ...
  const save = () => {
    ...
    setCatOrder(prev=>prev.includes(saved.cat1)?prev:[...prev,saved.cat1]);
    setItems((prev) => (selId === null ? [...prev, saved] : prev.map((it) => (it.id === saved.id ? saved : it))));
    // Standalone form can be the first browser entry: return inside the mock, not out of it.
    const listView: ViewSnap = { page: 'items', screen: 'list', itemId: null, sheet: false };
    window.history.replaceState(listView, '');
    setSelCat1(saved.cat1);
    applyView(listView);
    showToast(selId === null ? '진료 항목을 등록했어요.' : '진료 항목을 저장했어요.');
    if (d.kakaoOn) {
  ...
  const confirmDelete = () => {
    ...
    setItems((prev) => prev.filter((it) => it.id !== deleteId)); if (d?.id === deleteId) closeForm(); setDeleteId(null); showToast('진료 항목을 삭제했어요.');
  };
  ...
  const movePrice = (toId: number) => {
    ...
    if(d.prices.find(p=>p.id===toId)?.preset||d.prices.find(p=>p.id===dragPriceId)?.preset)return;
  ...
  const toggleGdVisible = (id: number) => setItems((prev) => prev.map((it) => {
    ...
    showToast(gdVisible ? '해당 진료 항목을 서비스에 노출합니다.' : '해당 진료 항목을 서비스에 미노출합니다.');
    return gdVisible
          ...
          <main className="cn-main rg-main tk-main">
            ...
            {page === 'items' && screen === 'list' && (
              <>
                <div className="cn-header tk-list-header">
                  <div>
                    <div className="cn-header-title">진료 항목</div>
                    <div className="tk-list-sub">굿닥에 노출되는 우리 병원 진료 항목과 가격 정보를 관리할 수 있어요. 카카오톡 예약하기에서도 보이는지 여기서 확인해요.</div>
                  </div>
                  <button className="tk-btn-primary" onClick={create}><PlusIcon /> 새 진료 항목</button>
                </div>
                ...
                <div className="tk-list-body" data-policy-id="gcp1-channel-overview">
                  <div className="tk-grid">
                    ...
                    <nav className="tk-grid-clist" aria-label="진료 항목 카테고리">
                      {cat1List.map((c) => (
                    ...
                    <section className="tk-grid-ilist">
                      {isCustom ? (
                        <>
                        {/* C안 — 미분류 재고는 이용자 검색에 안 잡힌다는 사실을 목록에서 알린다 */}
                        <div className="tc-list-banner">
                          <WarnIc />
                          <span>
                            <b>분류가 없어 이용자 검색에 노출되지 않는 항목 {customItems.length}개</b>예요. 항목을 열어 분류를 지정하면 카테고리 검색에 잡힙니다.
                          </span>
                        </div>
            ...
            {page === 'items' && screen === 'form' && d && (
              <>
                <div className="cn-header tk-form-header">
                  ...
                  <div className="rg-eyebrow">진료 항목 <span className="sep-preview-label">검토용 시안 · 실제 서비스 아님</span></div>
                  <div className="cn-header-title">{selId === null ? '진료 항목 등록' : '진료 항목 정보'}</div>
                </div>
                ...
                <div className="rg-container">
                  <div className="rg-form">
                    ...
                    <section className="rg-card required">
                      ...
                      <CategoryFirst value={d} onChange={changeCategory} />{errors.category&&<p className="rg-error" role="alert">{errors.category}</p>}
                      <div className="rg-field">
                        <FieldHead label="진료 항목명" helpers={['굿닥 이용자에게 보여질 이름을 입력해 주세요. 분류와 관계없이 필수 정보예요.']} />
                        <input aria-label="진료 항목명" className={`rg-input${errors.alias ? ' error' : ''}`} placeholder="예) 가다실 9가 접종, 영유아 검진" maxLength={ALIAS_MAX} value={d.alias} onChange={(e) => { patch({ alias: e.target.value }); clearErr('alias'); }} />
                          ...
                          <div className="rg-field price" data-policy-id="gcp1-kakao-price">
                            <div className="tc-price-head">
                              <FieldHead
                                label="가격 정보"
                                helpers={[
                                  d.fluPreset ? '기본 1회 가격을 입력해 주세요. 필요한 경우에만 옵션을 추가할 수 있어요.' : canonical
                                    ? '규격이 정해진 진료 항목이라 규격을 고르고 구성요소 금액만 입력해요.'
                                    : '이용자에게 보여줄 가격 정보를 설정해 주세요. (예: 횟수별, 시술명별 등)'
                                ]}
                              />
                              {schema && (
                                /* 프로토타입 전용 비교 장치. 실제로는 병원이 고르지 않는다 —
                                 * 진료 항목에 규격 스키마가 있으면 표준 상품, 없으면 자유 상품으로
                            ...
                            {canonical ? (
                              ...
                              {d.fluPreset?<div className="sep-fixed-unit">공통 단위 <strong>회</strong><span>추천 항목의 모든 가격 옵션에 적용돼요.</span></div>:<div className="tc-unit-bar">
                                <label className="tc-unit-label" htmlFor="common-unit">공통 단위</label>
                                <select id="common-unit" aria-label="공통 단위" className="rg-input" style={{width:180}} value={d.unit ?? NO_UNIT} onChange={e=>requestReset({unit:e.target.value},'단위')}>
                                ...
                                <span className="tc-unit-note">
                                  {(d.unit ?? NO_UNIT) === NO_UNIT
                                    ...
                                    : '옵션명은 수량과 단위로 표시돼요. 단위를 바꾸면 작성한 가격을 초기화해요. cc는 소수 입력이 가능해요.'}
                                </span>
                              </div>}
                              <div className="rg-price-list">
                                {d.prices.map((p,index) => (<React.Fragment key={p.id}>
                                  {d.fluPreset&&index===1&&<div className="sep-preset-title">추가 가격 옵션 <span>수량은 입력할 수 있고, 단위는 회로 고정돼요.</span></div>}
                                  <PriceRow
                                    key={p.id}
                                    p={p}
                                    unit={d.unit ?? NO_UNIT}
                      ...
                      {!(PRICE_SCHEMAS[lockedSchemaName] && priceForm === 'canonical') && (
                    ...
                    {pendingReset&&<PriceResetDialog kind={pendingReset.kind} onClose={()=>setPendingReset(null)} onConfirm={()=>applyReset(pendingReset.patch,pendingReset.kind)}/>} 
                    ...
                    <section className="rg-card extra" data-policy-id="gcp1-kakao-product-copy">
                      ...
                      <div className="rg-field" data-policy-id="gcp1-kakao-product-images">
                        <FieldHead label="대표 사진" optional helpers={['진료 항목을 대표하는 사진을 업로드해 주세요.']} />
                      ...
                      <div className="rg-field">
                        <FieldHead label="한 줄 소개" optional helpers={['진료 항목을 한눈에 이해할 수 있는 짧은 소개 문구를 입력해 주세요.']} />
                      ...
                      <div className="rg-field">
                        <FieldHead label="상세 소개" optional helpers={['진료 항목 상세 페이지에서 보여질 자세한 소개 내용을 입력해 주세요.']} />
                      ...
                      <div className="rg-field">
                        <FieldHead label="상세 소개 사진" optional helpers={[`진료 항목 상세 페이지에 노출할 사진을 업로드해 주세요. (최대 ${DETAIL_IMG_MAX}개)`, '권장 사이즈 가로 800px, 세로 15,000px 이하 · 파일당 최대 20MB · jpeg, jpg, png, gif', '이미지를 드래그해 순서를 바꿀 수 있어요.']} />
                      ...
                      <div className="rg-divider" />
                      <div className="rg-field">
                        <FieldHead label="진료 항목 키워드" optional helpers={['포털에서 더 잘 검색되도록 관련 키워드를 입력해 주세요.']} />
                    ...
                    {hospitalLinked && <section className="tk-platform-section" data-policy-id="gcp1-channel-visibility">
                      ...
                      <div className="rg-card tk-kcard">
                        ...
                        <div className="tk-kbody">
                          {!d.gdVisible ? (
                            <div className="tk-kpolicy-alert" role="note"><span className="tk-kpolicy-alert-ic"><CautionIc /></span><span>굿닥에 노출 중인 진료 항목만 카카오톡 예약하기에도 노출할 수 있어요.</span></div>
                          ) : !d.kakaoOn ? (
                            ...
                            <div className="tk-kauto" role="note"><span className="tk-kauto-ic"><CautionIc /></span><span className="tk-kauto-txt">위에 입력한 진료 항목 정보가 카카오톡 예약하기에도 함께 표시돼요.</span></div>
                          )}
                  ...
                  <GoodocPreview d={d} lockedSchemaName={lockedSchemaName} />
                </div>
                ...
                <div className="rg-footer">
                  ...
                  <div className="rg-footer-right">
                    {selId === null ? (
                      <>
                        <span className="rg-footer-label">굿닥 이용자에게 진료 항목을 노출하고 예약을 받습니다.</span>
                        <button className={`rg-toggle${d.gdVisible ? '' : ' off'}`} aria-label="굿닥 진료 항목 노출" aria-pressed={d.gdVisible} onClick={() => patch({ gdVisible: !d.gdVisible })}><span className="rg-toggle-knob" /></button>
                      </>
                    ) : (
            ...
            {deleteId !== null && (
              <div className="ap-dim" onClick={() => setDeleteId(null)}><div className="ap-modal set-modal" onClick={(e) => e.stopPropagation()}>
                <div className="ap-modal-title">{deleteTarget?.activeReservations ? '예약이 있는 진료 항목은 삭제할 수 없어요' : '해당 진료 항목을 삭제하시겠어요?'}</div>
            ...
            {leaveOpen && (
              <div className="ap-dim" onClick={() => setLeaveOpen(false)}><div className="ap-modal set-modal" onClick={(e) => e.stopPropagation()}>
                <div className="ap-modal-title">진료 항목 정보 입력을 그만하시겠어요?</div>
                <div className="set-modal-body">그만하면 지금까지 입력한 정보는 저장되지 않아요.</div>
                <div className="ap-modal-btns"><button className="rg-btn-cancel" onClick={() => setLeaveOpen(false)}>취소</button><button className="rg-btn-save" onClick={confirmLeave}>확인</button></div>
              </div></div>
            )}
            ...
            {catSheet && (() => {
              /* 닫기는 히스토리 뒤로 — popstate 가 같은 폼을 유지한 채 모달만 닫는다.
               * itemId 가 같으면 드래프트를 다시 클론하지 않으므로 입력값이 보존된다. */
                  ...
                  <div className="ap-modal tc-modal" onClick={(e) => e.stopPropagation()}>
                    ...
                    {sheetMode === 'step' && (
                      <div className="tc-modal-body">
                        {!sheetC1 ? (
                          <>
                            <div className="tc-sheet-note">대분류 {TAXONOMY.length}개 · 중분류 {total}개 — 굿닥 표준 진료 항목</div>
                            {TAXONOMY.map((a) => (
