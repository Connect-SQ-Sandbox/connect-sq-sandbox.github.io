...
| 화면 | 소스 | 산출물 | 설명 |
|---|---|---|---|
| **진료항목 분류 필드 분리(C안) + 카카오 연동** (신규) | `pages/connect/ti-category-c/index.page.tsx` | `out/ti-category-c.html` | `kakao-realcode-test` 복제 + **분류 필드 분리**만 추가(카카오 설정 카드·발문 빌더·예약 신청 내역·진료 예약 설정은 무수정). 진료항목명(자유 입력)과 분류(대분류 › 중분류)를 별도 필드로 분리 — 표준 항목 선택 시 동시 채움, 직접 입력 시 분류만 따로 선택. 역매칭 추천 원클릭, 분류 없을 때 검색 노출 손실 경고, 2단 분류 선택 시트(실제 마스터 대 15·중 69·소 187 전량), 상단 스위치로 분류 필수 정책 비교. |
