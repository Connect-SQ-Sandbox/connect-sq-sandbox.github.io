...
function ChannelMarks({ it, hospitalLinked, apptUsed }: { it: Item; hospitalLinked: boolean; apptUsed: boolean }) {
  ...
  const kakaoTitle = kakaoShown ? '카카오톡 예약하기에서 보임' : '카카오톡 예약하기에서 안 보임';
  return (
...
function ItemRow({ it, hospitalLinked, apptUsed, onOpen, onToggle, onDelete, onDragStart, onDrop }: { it: Item; hospitalLinked: boolean; apptUsed: boolean; onOpen: () => void; onToggle: () => void; onDelete: () => void; onDragStart: () => void; onDrop: () => void }) {
  return (
...
function ApptScreen({ appts, setAppts, hospitalLinked, failNextSync, consumeFailure, focusAdditionalToken, showToast }: {
  ...
  const rows = useMemo(() => {
    ...
    return r;
  }, [appts, tab, tabDef, statusFilter, preset, startDate, endDate, applied, searchType]);
        ...
        <div className={`ap-table${hospitalLinked ? '' : ' no-channel'}`}>
          ...
          {rows.length === 0 ? (
            ...
            <div key={a.id} className="ap-tr ap-row" onClick={() => setDetailId(a.id)}>
              <span className="ap-status-stack"><span className={`ap-tag ${AS_TAG[a.status] || 'gray'}`}>{AS_LABEL[a.status] || a.status}</span></span>
...
function SettingsScreen({ itemCount, operation, onGlobalChange, onSettingChange, showToast, onHours }: {
  itemCount: number;
      ...
      <div className="cn-screen">
        ...
        <div className="cn-body">
          ...
          <main className="cn-main rg-main tk-main">
            ...
            {/* ========================= 운영 설정 ========================= */}
            {page === 'settings' && <SettingsScreen itemCount={items.length} operation={operation} onGlobalChange={changeGlobalOperation} onSettingChange={changeOperationSetting} showToast={showToast} onHours={() => setPage('hours')} />}
                ...
                <div className="tk-list-body" data-policy-id="gcp1-channel-overview">
                  <div className="tk-grid">
                    ...
                    <section className="tk-grid-ilist">
                      {isCustom ? (
                        <div className="tk-l2-body">{customItems.map((it) => (<ItemRow key={it.id} it={it} hospitalLinked={hospitalLinked} apptUsed={operation.apptUsed} onOpen={() => open(it)} onToggle={() => toggleGdVisible(it.id)} onDelete={() => setDeleteId(it.id)} onDragStart={() => setDragItemId(it.id)} onDrop={() => moveItem(it.id)} />))}</div>
                      ) : (
                        groups.map((g) => (
                          <div key={g.name} className="tk-l2" draggable onDragStart={() => setDragGroup(g.name)} onDragOver={(e) => e.preventDefault()} onDrop={() => moveGroup(g.name)}>
                            ...
                            <div className="tk-l2-body">{g.items.map((it) => (<ItemRow key={it.id} it={it} hospitalLinked={hospitalLinked} apptUsed={operation.apptUsed} onOpen={() => open(it)} onToggle={() => toggleGdVisible(it.id)} onDelete={() => setDeleteId(it.id)} onDragStart={() => setDragItemId(it.id)} onDrop={() => moveItem(it.id)} />))}</div>
                                        ...
                                        <div className="tk-q-opts">
                                          {q.options.map((option, optionIndex) => (
                                            <div key={optionIndex} className="tk-q-optrow">
                                              ...
                                              <input className={`rg-input${errors[`q-${q.id}-options`] && !option.trim() ? ' error' : ''}`} placeholder={`항목 ${optionIndex + 1}`} maxLength={K_Q_OPT_LEN_MAX} value={option} onChange={(e) => setOpt(q.id, optionIndex, e.target.value)} />
