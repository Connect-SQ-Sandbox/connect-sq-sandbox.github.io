...
| 화면 | 소스 | 산출물 | 설명 |
|---|---|---|---|
| **진료항목명 프리필 규칙 비교 랩** (신규) | `ti-bulk-schema/ti-prefill-lab.html` | `out/ti-prefill-lab.html` | 분류를 고르면 진료항목명을 프리필하는데, 병원이 이름을 고친 뒤 분류를 다시 바꾸면 어떻게 할지가 미정입니다. 후보 3안을 **같은 조작에 동시에** 돌려 비교하는 판입니다 — **A 현재 구현**(손대면 영구 유지) · **B 항상 덮어쓰기** · **C 접두 치환**(이름이 이전 소분류명으로 시작하면 그 앞부분만 교체하고 뒤 수식어는 남김). 유형 7종을 버튼으로 재현하고(수식어 덧붙임 / 전혀 다른 이름 / 띄어쓰기 차이 / 대분류 이동 / 기타 변경 / 편집 중 변경) 조작 기록 표로 갈라지는 지점을 봅니다. 기타를 고를 때 C규칙이 비울지 유지할지도 토글로 시험합니다. 마스터에서 대분류 3개(소분류 23 + 기타)만 발췌했습니다. |
