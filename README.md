...
| **신청 웹 · 독감 무료접종 대상자 선택 UX** (신규) | `pages/connect/appt-vaccine-target/index.page.tsx` | `out/appt-vaccine-target.html` | 환자 신청 웹에서 **무료 백신 대상자**(임신부·어린이·어르신)로 신청할 때의 **독립된 결정 2건**을 최상단 `이슈` 세그먼트로 갈라 비교한다(화면당 변수 1개). **이슈 ① 하위 뎁스 처리**(안 A~E) / **이슈 ② 진료 대상자**(F-1~F-4, 안은 A 고정). 각 이슈 안에서 ① **비교 보드** — **A~E는 추천 순위**(2026-09-16 세화님): A 하위 뎁스 접기 / B 읽기 전용 행 / C 비활성 / D 사전 분기(바텀시트) / E 현행 유지. 원본 캔버스 이름과 letter가 다르다. 상태 ①대상자 선택 전 ②해당 없음 ③어린이 + 예약 정보 확인 카드까지 나란히 놓고, 탭 수·허위 선택 인상 위험·레이아웃 흔들림·설명 텍스트 필요량·재사용도 5축으로 비교한다. ② **실제 동작**(기본 진입) — 상단 `안` 세그먼트로 A~E를, `진료 대상자` 세그먼트로 F-1~F-4를 바꿔 가며 직접 눌러보는 인터랙티브 화면. 대상자를 고르면 제품·금액은 `병원 상담 후 결정`·`미정`이 되고 판정·서류·인증 UI 없이 고지 문구로만 처리한다. ③ **부록 · 진료 대상자 처리** — 예약자 본인이 아닌 사람의 접종을 신청하는 경우 4안. F-1 체크 비활성 / F-2 체크 후 해제는 **생년월일 대조 = 불일치를 오류로** 다루는 방식(비교용 보존), F-3 대상자 선택 화면 통합형 / F-4 신청서 섹션 재설계형은 **대조를 버리고 본인/대리를 먼저 묻는** 방식이다 — 어린이·영유아는 부모가 자녀를 대신 예약하는 것이 기본 케이스라 오류 프레이밍이 틀렸다는 판단(2026-09-16). 병원(어드민) 쪽 짝 화면은 트리판 t4.2 `[T45]`. Claude Design 핸드오프 `무료접종 대상자 선택 UX.dc.html`(2026-09-16)을 React로 이식. **채택안 미정.** |
